...
import { ACCOUNTDETAILS, CREDITNOTE, CUSTOMER, PARTICULARS, Purchase } from '../model/creditnote.interface';
...
@Component({
  selector: 'dfinance-frontend-debitnote',
...
export class DebitnoteComponent {
  ...
  allDebitNoteList=[] as Array<CREDITNOTE>;   //store leftside data from api
  tempDebitNoteList : any =[];   //temperory store
  firstDebitNote!:number;        //store first data

  //Initailization
  selectedDebitId!:number;   //store id from the click in left side
  pageId=0;
  voucherNo = 0;
  vocherName = "";
  selectedPartyId = 0;
 ...
 //supplier
  SupplierData=[] as Array<CUSTOMER>;
  updatedSupplier='';
  supplierreturnField='id';
  supplierKeys =['AccountCode','AccountName','Address'];
  selectedSupplierId=0;
  supplierExcludekeys=['id']
  ...
  enableFormControls() {
    this.DebitNoteForm.get('voucherdate')?.enable();
    this.DebitNoteForm.get('customer')?.enable();
    this.DebitNoteForm.get('customername')?.enable();
    this.DebitNoteForm.get('reference')?.enable();
    this.DebitNoteForm.get('particulars')?.enable();
    this.DebitNoteForm.get('narration')?.enable();
    
  }

   disbaleFormControls() {
    this.DebitNoteForm.get('voucherdate')?.disable();
    this.DebitNoteForm.get('customer')?.disable();
    this.DebitNoteForm.get('customername')?.disable();
    this.DebitNoteForm.get('reference')?.disable();
    this.DebitNoteForm.get('particulars')?.disable();
    this.DebitNoteForm.get('narration')?.disable();
  }
  ...
  fetchAccountDetails() {
    this.creditNoteService.getDetails(EndpointConstant.FILLACCOUNTDATA)
      .subscribe({
        ...
        complete: () => {
          // Optionally, log completion of the observable
...
convertToDateInputFormat(dateString: string): string {
  ...
  // onKeyDowns(event: KeyboardEvent, picker: any) {
  //   if (event.key === "PageDown") {
  //     event.preventDefault(); 
  //     if (picker) {
  //       picker.open(); 
  //     } else {
  //       console.error('Picker is undefined');
  //     }
  //   }
  ...
  //  checkAccountCodeCanAdd() {
  //   const custId = this.CreditNoteForm.get('customer')?.value;
  //   if (custId) {
  //     this.fetchAccountDetails();
  //   }
  // }
  ...
  //                 //     this.tempFillDetails[this.currentRowIndex][columnKeyName]['duedate'] = event.key;
  //                 //   } else if(columnName == 'debit'){
  //                 //     this.tempFillDetails[this.currentRowIndex][columnKeyName]['debit'] = event.key;
  //                 //   }  else{
  ...
  fetchAllCreditNote():void
  {
    ...
    .subscribe({
      next: (response) => {
        this.isLoading = false;
        this.allDebitNoteList = response?.data;
       // this.tempCreditNoteList =  this.allCreditNoteList[0].ID;
       this.tempDebitNoteList = [...this.allDebitNoteList];
        this.firstDebitNote = this.allDebitNoteList[0].ID;
      },
      error: (error) => {
        this.isLoading = false;
        console.error('An Error Occured', error);
  ...
  onClickCreditNote(event:any): void
  {
    if(event.type === 'click')
    {
      this.selectedDebitId = event.row.ID;
      this.fetchAllCreditNoteById();
    }
  }
  ...
  fetchAllCreditNoteById():void{
    this.creditNoteService
    .getDetails(EndpointConstant.FILLPURCHASEBYID + this.selectedDebitId + '&pageId=' + this.pageId)
    .pipe(takeUntil(this.destroySubscription))
    .subscribe({
      next: (response) => {
        
        this.currentPurchase = response?.data;
        //console.log("checking!!!!!!!!!!!!!!"+JSON.stringify(this.currentPurchase, null, 2));
  ...
  //   this.noGridItem = true;
  //   this.accountDetailsObj = {};
  //   this.tempAccFillDetails = [];  // Clear tempFillDetails for fresh data
  // }



 // fillAccData = [] as Array<FillTransactionEntries>;

  


  onCustomerSelected(option: any): any {
    this.selectedSupplierId = option;
    this.SupplierData.forEach((item) => {
      if (item.id === option) {
        ...
        this.updatedSupplier = item.name;
        ...
        //console.log("selcustOBJ=>"+this.selectedSupplierObj)
        this.checkAdvanceAmount(this.selectedSupplierObj.id);
...
onClickSaveCreditNote(){
  if(this.DebitNoteForm.get('customer')?.value == null){
    alert('Party is Mandatory');
    this.moveFocusToDropdown('customer');
    return false;
  } 

  const date=this.DebitNoteForm.get('voucherdate')?.value
  if(date === null){
  ...
  if(this.accDetails[0].accountCode.credit === null || this.accDetails[0].accountCode.credit === "" ){
    alert("Amount cannot be null. Please enter amount");
    return false;
  }
  //set particular data..
  if (this.DebitNoteForm.get('particulars')?.value) {
  const selectedParticularId = this.DebitNoteForm.get('particulars')?.value;
...
// console.log(this.tempFillDetails);
 ...
 const payload = {
   "id": this.selectedDebitId ? this.selectedDebitId : 0,
   "voucherNo": this.formVoucherNo,
   "voucherDate": this.DebitNoteForm.value.voucherdate,
   "reference": this.DebitNoteForm.value.reference,
   "party":this.selectedSupplierObj,
   "particulars":this.selectedParticularrsObj,
   "narration":this.DebitNoteForm.value.narration,
...
this.creditNoteService.saveDetails(EndpointConstant.SAVECREDIT + this.pageId+ '&VoucherId=' +this.voucherNo, payload)
  ...
  .subscribe({
    ...
    //   alert('Successfully saves debitnote');
      const status = response.httpCode as keyof typeof STATUS_MESSAGES; 
      const message =STATUS_MESSAGES[status] ; 
      alert(message); 
      this.selectedDebitId= this.firstDebitNote;
  ...
  // onChangecredit(rowIndex: any, event: any) {
  ...
  addRow(itemcodesel = false,event?: KeyboardEvent) {
    ...
    //console.log("allItemCodesFilled"+JSON.stringify(allItemCodesFilled,null,2))
  ...
  onParticularSelect(): void {
    this.selectedParId = this.DebitNoteForm?.get('particulars')?.value;
  ...
  confirmDelete() {
    this.creditNoteService.deleteDetails(EndpointConstant.DELETEPURCHASE + this.selectedDebitId + '&pageId=' + this.pageId)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
  ...
  confirmCancel() {
    this.creditNoteService.updateDetails(EndpointConstant.CANCELPURCHASE + this.selectedDebitId + '&reason=' + this.cancelReason + '&pageId=' + this.pageId, {})
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          const status = response.httpCode as keyof typeof STATUS_MESSAGES;
...
//   if (this.tempAccFillDetails?.credit && keyboardEvent.key === 'Tab') {
...
onClickAdvanceAmountOption(event: KeyboardEvent, rowIndex: number): void { 
  ...
  checkAdvanceAmount(a:number) {
    let voucherDate = this.DebitNoteForm.get('voucherdate')?.value;
    if (voucherDate == null) {
      alert('Invalid Date');
      return false;
    }
    voucherDate = this.datePipe.transform(voucherDate, 'MM-dd-YYYY');
    let creditId = this.selectedSupplierId ? this.selectedSupplierId : 0;
    this.creditNoteService
        ...
        next: (response) => {
          console.log("BILLANDREF=>"+EndpointConstant.FILLADVANCE + creditId + '&voucherId=' + this.voucherNo + '&date=' + voucherDate)
            this.advanceAmountObj = response?.data;
             ...
             console.log("advanceAmountObj="+JSON.stringify(this.advanceAmountObj))
  ...
  onChangecredit(rowIndex: any, event: any) {
    ...
    this.accDetails[rowIndex]['credit'] = debit;
