...
import { additonalChargesPopup, bankPopup, cardPopup, cashPopup, chequePopup, Currency, DeliveryLocation, GridSettings, ItemOptions, Items, itemTransaction, PayType, Projects, Purchase, Purchases, Reference, Salesman, SizeMaster, StockItems, Supplier, taxPopup, UnitPricePopup, VehicleNo, VoucherType, Warehouse } from '../model/purchase.interface';
...
@Component({
  selector: 'dfinance-frontend-purchaserequest',
...
export class PurchaserequestComponent {
  ...
  ngOnInit(): void {
    this.purchaseOderForm = this.formBuilder.group({
    ...
    this.purchaseOderForm.valueChanges.subscribe(() => {
      if(this.purchaseOderForm.dirty){
  ...
  onClickNewPurchaseOrder() {
    ...
    if (this.isInputDisabled == true) {
      ...
      this.selectedPurchaseId = 0;
      this.purchaseOderForm.patchValue({
  ...
  fetchItemFillData() {
    this.partyId = this.selectedPartyId;
    this.locId = this.purchaseOderForm.get('warehouse')?.value;
    this.PurchaseService
  ...
  enableFormControls() {
    this.purchaseOderForm.get('purchasedate')?.enable();
    this.purchaseOderForm.get('reference')?.enable();
    this.purchaseOderForm.get('supplier')?.enable();
    this.purchaseOderForm.get('currency')?.enable();
    this.purchaseOderForm.get('exchangerate')?.enable();
    this.purchaseOderForm.get('warehouse')?.enable();
    this.purchaseOderForm.get('project')?.enable();
    this.purchaseOderForm.get('description')?.enable();
    this.purchaseOderForm.get('approve')?.enable();
    this.purchaseOderForm.get('terms')?.enable();
    this.purchaseOderForm.get('totaldiscpercent')?.enable();
    this.purchaseOderForm.get('discountamount')?.enable();
    this.purchaseOderForm.get('roundoff')?.enable();
    this.purchaseOderForm.get('netamount')?.enable();
    this.purchaseOderForm.get('grandtotal')?.enable();
    // this.purchaseOderForm.get('paytype')?.enable();
    this.purchaseOderForm.get('totalpaid')?.enable();
    this.purchaseOderForm.get('balance')?.enable();
    this.purchaseOderForm.get('duedate')?.enable();
    this.purchaseOderForm.get('additemcharges')?.enable();
    this.purchaseOderForm.get('salesman')?.enable();
    this.purchaseOderForm.get('vatno')?.enable();
    this.purchaseOderForm.get('partyinvoiceno')?.enable();
    this.purchaseOderForm.get('partyinvoicedate')?.enable();
  ...
  disbaleFormControls() {
    this.purchaseOderForm.get('purchasedate')?.disable();
    this.purchaseOderForm.get('reference')?.disable();
    this.purchaseOderForm.get('supplier')?.disable();
    this.purchaseOderForm.get('currency')?.disable();
    this.purchaseOderForm.get('exchangerate')?.disable();
    this.purchaseOderForm.get('warehouse')?.disable();
    this.purchaseOderForm.get('project')?.disable();
    this.purchaseOderForm.get('description')?.disable();
    this.purchaseOderForm.get('approve')?.disable();
    this.purchaseOderForm.get('terms')?.disable();
    this.purchaseOderForm.get('totaldiscpercent')?.disable();
    this.purchaseOderForm.get('discountamount')?.disable();
    this.purchaseOderForm.get('roundoff')?.disable();
    this.purchaseOderForm.get('netamount')?.disable();
    this.purchaseOderForm.get('grandtotal')?.disable();
    // this.purchaseOderForm.get('paytype')?.disable();
    this.purchaseOderForm.get('totalpaid')?.disable();
    this.purchaseOderForm.get('balance')?.disable();
    this.purchaseOderForm.get('duedate')?.disable();
    this.purchaseOderForm.get('additemcharges')?.disable();
    this.purchaseOderForm.get('salesman')?.disable();
    this.purchaseOderForm.get('vatno')?.disable();
    this.purchaseOderForm.get('partyinvoiceno')?.disable();
    this.purchaseOderForm.get('partyinvoicedate')?.disable();
  ...
  onClickSavePurchaseOrder() {
    if(this.purchaseOderForm.get('supplier')?.value == null){
    ...
    //fetch project data...
     if (this.purchaseOderForm.value.project) {
      this.projectData.forEach((element:any)=>{
        if(element.projectname == this.purchaseOderForm.value.project){
    ...
    //set warehouse data..
    if(this.purchaseOderForm.get('warehouse')?.value){
      this.warehouseData.forEach((element:any)=>{
        if(element.id == this.purchaseOderForm.get('warehouse')?.value){
    ...
    //set salesman data...
    if(this.purchaseOderForm.get('salesman')?.value){
      this.salesmanData.forEach((element:any)=>{
        if(element.name == this.purchaseOderForm.get('salesman')?.value){
    ...
    const payload = {
      ...
      "date": this.purchaseOderForm.value.purchasedate,
      "reference": this.purchaseOderForm.value.reference,
      ...
      "description":  this.purchaseOderForm.value.description,
      "grossAmountEdit": this.isgrossAmountEditable,
      "fiTransactionAdditional": {
        "transactionId": 0,
        "terms": this.purchaseOderForm.value.terms,
        ...
        "approve": this.purchaseOderForm.value.approve,
        "days": 0,
        "closeVoucher": true,
        "code": "string",
      ...
      "transactionEntries": {
       "totalDisc": this.discountTotal,
        ...
        "netAmount": this.purchaseOderForm.value.netamount,
        "grandTotal": this.grandTotal,
        "payType": this.selectedPayTypeObj,
        "dueDate": this.purchaseOderForm.value.duedate,
        "totalPaid": this.purchaseOderForm.value.totalpaid,
        "balance": this.purchaseOderForm.value.balance,
  ...
  updateCallback(payload: any) {
    this.PurchaseService.updateDetails(EndpointConstant.UPDATEPURCHASEORDER+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
          if(!this.activePrintOption){
            this.baseService.showCustomDialogue('Success');
          } else{
  ...
  createCallback(payload: any) {
    this.PurchaseService.saveDetails(EndpointConstant.SAVEPURCHASEORDER+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  onClickDeletePurchaseOrder(event:Event) {
  ...
  checkItemCodeCanAdd() {
    const warehouseId = this.purchaseOderForm.get('warehouse')?.value;
    const supplierId = this.purchaseOderForm.get('supplier')?.value;
  ...
  onChangeTaxType(): void {
    const selectedTaxTypeId = this.purchaseOderForm.get('taxtype')?.value;
  ...
  onChangeQuality(): void {
    const selectedQualityId = this.purchaseOderForm.get('quality')?.value;
  ...
  onStockItemChange(event: Event) {
    ...
    if (this.isStockItem == false) {
      this.purchaseOderForm.get('invaccount')?.enable();
      this.purchaseOderForm.get('salesaccount')?.enable();
      this.purchaseOderForm.get('costaccount')?.enable();
      this.purchaseOderForm.get('purchaseaccount')?.enable();
    } else {
      this.purchaseOderForm.get('invaccount')?.disable();
      this.purchaseOderForm.get('salesaccount')?.disable();
      this.purchaseOderForm.get('costaccount')?.disable();
      this.purchaseOderForm.get('purchaseaccount')?.disable();
    }
  }
  ...
  translateItemName() {
    let itemName = this.purchaseOderForm.get('itemname')?.value;
  ...
  checkAdvanceAmount() {
    let voucherDate = this.purchaseOderForm.get('purchasedate')?.value;
    if (voucherDate == null) {
  ...
  onClickRoundOff(){
    this.roundValue = Number(this.purchaseOderForm.get('roundoff')?.value);
 ...
 @HostListener('window:keydown', ['$event'])
  onBodyKeydown(event: KeyboardEvent): void {
    if (event.ctrlKey && (event.key === 'p' || event.key === 'P')) {
      ...
      if(!this.isDeleteBtnDisabled){
        this.onClickDeletePurchaseOrder(event);
      }      
  ...
  setMaxHeight(): void {
    ...
    const sectionContentClass = document.querySelectorAll('.po-section .section-content');
    let heightDiff = 0;
