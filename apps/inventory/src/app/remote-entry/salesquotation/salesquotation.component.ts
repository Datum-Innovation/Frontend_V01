...
@Component({
  selector: 'dfinance-frontend-salesquotation',
  templateUrl: './salesquotation.component.html',
  styleUrls: ['./salesquotation.component.scss'],
  providers: [
...
export class SalesquotationComponent {
  ...
  updateCallback(payload: any) {
    this.PurchaseService.updateDetails(EndpointConstant.UPDATESALESEQUOTATION+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  createCallback(payload: any) {
    this.PurchaseService.saveDetails(EndpointConstant.SAVESALESEQUOTATION+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
 ...
 @HostListener('window:keydown', ['$event'])
  ...
  showCustomDialogue(message:any,title:any = '',inputKey:any = 'custom'){
  ...
  setMaxHeight(): void {
    ...
    const sectionContentClass = document.querySelectorAll('.sales-quotation-section .section-content');
    let heightDiff = 0;
