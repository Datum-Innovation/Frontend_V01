...
import { additonalChargesPopup, bankPopup, cardPopup, cashPopup, chequePopup, Currency, Customer, DeliveryLocation, GridSettings, ItemOptions, Items, itemTransaction, PayType, Projects, Purchase, Purchases, Reference, Salesman, SizeMaster, StockItems, Supplier, taxPopup, UnitPopup, UnitPricePopup, VehicleNo, VoucherType, Warehouse } from '../model/purchase.interface';
...
@Component({
  selector: 'dfinance-frontend-salesestimate',
  templateUrl: './salesestimate.component.html',
  styleUrls: ['./salesestimate.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: PickDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: PICK_FORMATS }
  ]
})
export class SalesestimateComponent {
  ...
  allSalesListEstimate = [] as Array<Purchases>;
  tempSalesList: any = [];
  ...
  ngOnInit(): void {
    this.salesEstimateForm = this.formBuilder.group({
    ...
    this.salesEstimateForm.valueChanges.subscribe(() => {
      if (this.salesEstimateForm.dirty) {
    ...
    this.subscription = interval(60000) // 5 minutes in milliseconds
      .subscribe(() => {
        this.fetchAllSalesEstimate(); // Call the function every 5 minutes
      });
  }
      ...
      .subscribe({
        next: (response) => {
          ...
          if (Array.isArray(response?.data)) {
            this.allSalesListEstimate = response?.data;
            this.tempSalesList = [...this.allSalesListEstimate];
            this.firstSales = this.allSalesListEstimate[0].ID;
          }
        },
        error: (error) => {
          // this.isLoading = false;
  ...
  onClickNewSaleEstimate() {
    ...
    if (this.isInputDisabled == true) {
      ...
      this.selectedSalesId = 0;
      this.salesEstimateForm.patchValue({
  ...
  onChangePayType() {
    let paytypeId = this.salesEstimateForm.get('paytype')?.value;
  ...
  fetchItemFillData() {
    this.partyId = this.selectedPartyId;
    this.locId = this.salesEstimateForm.get('warehouse')?.value;
    this.PurchaseService
  ...
  enableFormControls() {
    this.salesEstimateForm.get('purchasedate')?.enable();
    this.salesEstimateForm.get('reference')?.enable();
    this.salesEstimateForm.get('customer')?.enable();
    this.salesEstimateForm.get('currency')?.enable();
    this.salesEstimateForm.get('exchangerate')?.enable();
    this.salesEstimateForm.get('warehouse')?.enable();
    this.salesEstimateForm.get('project')?.enable();
    this.salesEstimateForm.get('description')?.enable();
    this.salesEstimateForm.get('approve')?.enable();
    this.salesEstimateForm.get('terms')?.enable();
    this.salesEstimateForm.get('totaldiscpercent')?.enable();
    this.salesEstimateForm.get('discountamount')?.enable();
    this.salesEstimateForm.get('roundoff')?.enable();
    this.salesEstimateForm.get('netamount')?.enable();
    this.salesEstimateForm.get('grandtotal')?.enable();
    this.salesEstimateForm.get('paytype')?.enable();
    this.salesEstimateForm.get('totalpaid')?.enable();
    this.salesEstimateForm.get('balance')?.enable();
    this.salesEstimateForm.get('duedate')?.enable();
    this.salesEstimateForm.get('additemcharges')?.enable();
    this.salesEstimateForm.get('salesman')?.enable();
    this.salesEstimateForm.get('vatno')?.enable();
    this.salesEstimateForm.get('partyinvoiceno')?.enable();
    this.salesEstimateForm.get('partyinvoicedate')?.enable();
  ...
  disbaleFormControls() {
    this.salesEstimateForm.get('purchasedate')?.disable();
    this.salesEstimateForm.get('reference')?.disable();
    this.salesEstimateForm.get('customer')?.disable();
    this.salesEstimateForm.get('currency')?.disable();
    this.salesEstimateForm.get('exchangerate')?.disable();
    this.salesEstimateForm.get('warehouse')?.disable();
    this.salesEstimateForm.get('project')?.disable();
    this.salesEstimateForm.get('description')?.disable();
    this.salesEstimateForm.get('approve')?.disable();
    this.salesEstimateForm.get('terms')?.disable();
    this.salesEstimateForm.get('totaldiscpercent')?.disable();
    this.salesEstimateForm.get('discountamount')?.disable();
    this.salesEstimateForm.get('roundoff')?.disable();
    this.salesEstimateForm.get('netamount')?.disable();
    this.salesEstimateForm.get('grandtotal')?.disable();
    this.salesEstimateForm.get('paytype')?.disable();
    this.salesEstimateForm.get('totalpaid')?.disable();
    this.salesEstimateForm.get('balance')?.disable();
    this.salesEstimateForm.get('duedate')?.disable();
    this.salesEstimateForm.get('additemcharges')?.disable();
    this.salesEstimateForm.get('salesman')?.disable();
    this.salesEstimateForm.get('vatno')?.disable();
    this.salesEstimateForm.get('partyinvoiceno')?.disable();
    this.salesEstimateForm.get('partyinvoicedate')?.disable();
  ...
  onClickSaveSalesEstimate() {
    if (this.salesEstimateForm.get('customer')?.value == null) {
    ...
    //fetch project data...
    if (this.salesEstimateForm.value.project) {
      this.projectData.forEach((element: any) => {
        if (element.projectname == this.salesEstimateForm.value.project) {
    ...
    //set warehouse data..
    if (this.salesEstimateForm.get('warehouse')?.value) {
      this.warehouseData.forEach((element: any) => {
        if (element.id == this.salesEstimateForm.get('warehouse')?.value) {
    ...
    //set salesman data...
    if (this.salesEstimateForm.get('salesman')?.value) {
      this.salesmanData.forEach((element: any) => {
        if (element.name == this.salesEstimateForm.get('salesman')?.value) {
    ...
    // if(this.selectedPayTypeObj.value == 'Cash' && this.balanceAmount > 0){
    ...
    const payload = {
      ...
      "date": this.salesEstimateForm.value.purchasedate,
      "reference": this.salesEstimateForm.value.reference,
      ...
      "description": this.salesEstimateForm.value.description,
      "grossAmountEdit": this.isgrossAmountEditable,
      "fiTransactionAdditional": {
        "transactionId": 0,
        "terms": this.salesEstimateForm.value.terms,
        ...
        "approve": this.salesEstimateForm.value.approve,
        "days": 0,
        "closeVoucher": true,
        "code": "string",
      ...
      "transactionEntries": {
        ...
        "netAmount": this.salesEstimateForm.value.netamount,
        "grandTotal": this.grandTotal,
        "payType": this.selectedPayTypeObj,
        "dueDate": this.salesEstimateForm.value.duedate,
        "totalPaid": this.salesEstimateForm.value.totalpaid,
        "balance": this.salesEstimateForm.value.balance,
  ...
  updateCallback(payload: any) {
    this.PurchaseService.updateDetails(EndpointConstant.UPDATESALESESTIMATE + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  createCallback(payload: any) {
    this.PurchaseService.saveDetails(EndpointConstant.SAVESALESESTIMATE + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  onClickDeleteSalesEstimate(event: Event) {
  ...
  onClickCancelSalesEstimate(event: Event) {
    event.preventDefault();
    if (!this.isCancel) {
      this.baseService.showCustomDialogue('Permission Denied!');
      return false;
  ...
  onClickSalesEstimate(event: any): void {
    if (event.type === 'click') {
  ...
  FillPurchaseDetails() {
    ...
    let payment: any = this.currentSalesEstimate?.payment.data;
    let chequeInfo: any = this.currentSalesEstimate?.cheque.data;
  ...
  setPayTypeBasedOnCustomer() {
    ...
    if (this.updatedCustomer == 'Cash Customer') {
      ...
      // Ensure payTypeObj is an array and handle its structure
      const payTypeArray = Array.isArray(this.payTypeObj)
        ? this.payTypeObj as PayType[] // Use type assertion here
        : Object.values(this.payTypeObj || {}) as PayType[];

      // Attempt to find the pay type with the name 'Cash'
      const foundPayType = payTypeArray.find((item: any) => item.name === 'Cash');

      // Get the id if the object is found, or set it to `null` otherwise
      const cashTypeId = foundPayType ? foundPayType.id : null;

      // Patch the form with the found ID or null
      this.salesEstimateForm.patchValue({
      "paytype": cashTypeId
      });
    } else {
  ...
  checkItemCodeCanAdd() {
    const warehouseId = this.salesEstimateForm.get('warehouse')?.value;
    const customerId = this.salesEstimateForm.get('customer')?.value;
  ...
  onChangeTaxType(): void {
    const selectedTaxTypeId = this.salesEstimateForm.get('taxtype')?.value;
  ...
  onChangeQuality(): void {
    const selectedQualityId = this.salesEstimateForm.get('quality')?.value;
  ...
  onStockItemChange(event: Event) {
    ...
    if (this.isStockItem == false) {
      this.salesEstimateForm.get('invaccount')?.enable();
      this.salesEstimateForm.get('salesaccount')?.enable();
      this.salesEstimateForm.get('costaccount')?.enable();
      this.salesEstimateForm.get('purchaseaccount')?.enable();
    } else {
      this.salesEstimateForm.get('invaccount')?.disable();
      this.salesEstimateForm.get('salesaccount')?.disable();
      this.salesEstimateForm.get('costaccount')?.disable();
      this.salesEstimateForm.get('purchaseaccount')?.disable();
    }
  }
  ...
  translateItemName() {
    let itemName = this.salesEstimateForm.get('itemname')?.value;
  ...
  filterSales(event: any) {
    ...
    // update the rows
    this.allSalesListEstimate = temp;
    // Whenever the filter changes, always go back to the first page
    //this.table.offset = 0;
  }
  ...
  checkAdvanceAmount() {
    let voucherDate = this.salesEstimateForm.get('purchasedate')?.value;
    if (voucherDate == null) {
  ...
  onClickRoundOff() {
    this.roundValue = Number(this.salesEstimateForm.get('roundoff')?.value);
  ...
  onBodyKeydown(event: KeyboardEvent): void {
    if (event.ctrlKey && (event.key === 'p' || event.key === 'P')) {
      ...
      if (!this.isDeleteBtnDisabled) {
        this.onClickDeleteSalesEstimate(event);
      }
  ...
  setMaxHeight(): void {
    ...
    const sectionContentClass = document.querySelectorAll('.sales-estimate-section .section-content');
    let heightDiff = 0;
    ...
    this.tableHeight = availableHeight - heightDiff - 102;
  }
