...
@Component({
  selector: 'dfinance-frontend-deliveryout',
...
export class DeliveryoutComponent {
  ...
  updateCallback(payload: any) {
    this.PurchaseService.updateDetails(EndpointConstant.UPDATEDELIVERYOUT + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  createCallback(payload: any) {
    this.PurchaseService.saveDetails(EndpointConstant.SAVEDELIVERYOUT + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  setMaxHeight(): void {
    ...
    const sectionContentClass = document.querySelectorAll('.delivery-out-section .section-content');
    let heightDiff = 0;
