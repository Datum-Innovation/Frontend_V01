...
@Component({
  selector: 'dfinance-frontend-salesorder',
...
export class SalesorderComponent {
      ...
      this.PurchaseService
        ...
        .subscribe({
          next: (response) => {
            ...
            if (Array.isArray(this.gridsettings )) {
  ...
  updateCallback(payload: any) {
    this.PurchaseService.updateDetails(EndpointConstant.UPDATESALESORDER + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  createCallback(payload: any) {
    this.PurchaseService.saveDetails(EndpointConstant.SAVESALESORDER + this.pageId + '&voucherId=' + this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  //   console.log("test sales id in print"+selectedSalesId)
  //   this.PurchaseService
  //     .getDetails(EndpointConstant.FILLPURCHASEBYID + selectedSalesId + '&pageId=' + this.pageId)
  //     .pipe(takeUntil(this.destroySubscription))
  //     .subscribe({
  //       next: async (response: any) => { // Explicitly define response type (can be typed if known)
  ...
  //             margin: 2,
  //             width: 200, // Increased QR code width for better visibility
  //             color: {
  //               dark: '#000000',
  //               light: '#ffffff'
  //             }
  ...
  //           this.PurchaseService.getDetails(EndpointConstant.FILLPARTYBALANCE + this.accid)
  //             .pipe(takeUntil(this.destroySubscription))
  //             .subscribe({
  //               next: (response: any) => {
  ...
  //                       CR NO. 2050074943 VAT NO. 311547462100003
  //                     </div>
  ...
  //                 // Create and use iframe for printing
  ...
  //                 // Add logo to the PDF
  //                 doc.addImage('/assets/SASTC.png', 'PNG', 70, 10, 60, 20);  // Logo
  //                 doc.text("شركة صالح التجارية ذ.م.م", 105, 40, { align: "center" });
  //                 doc.text("Saleh Trading Company L.L.C", 105, 45, { align: "center" });

  //                 // Add invoice details
  //                 doc.text(`Date: ${new Date(data.transaction.fillTransactions.date).toLocaleString()}`, 105, 55, { align: "center" });
  //                 doc.text(`Invoice No: ${data.transaction.fillTransactions.transactionNo}`, 105, 60, { align: "center" });
  //                 doc.text(`Customer: ${data.transaction.fillTransactions.accountName}`, 105, 65, { align: "center" });
  //                 doc.text(`Customer VAT No: ${data.transaction.fillTransactions.vatNo || "Not Available"}`, 105, 70, { align: "center" });

  //                 // Table of items
  //                 const yPosition = 80;
  //                 const table = doc.autoTable({
  //                   startY: yPosition,
  //                   head: [['SI', 'ITEM', 'QTY', 'PRICE', 'AMOUNT']],
  //                   body: data.transaction.fillInvTransItems.map((item: any, index: number) => [
  //                     index + 1,
  //                     item.itemName || item.arabicName,
  //                     `${item.qty} (${item.unit || item.unitArabic})`,
  //                     item.rate,
  ...
  //                 // Format amounts and balance for display
  //                 // Calculate totals
  //                 // const amountStr = amount.toFixed(2);
  //                 // const taxStr = tax.toFixed(2);
  //                 // const totalStr = total.toFixed(2);
  //                 // const balanceStr = balanceData.toFixed ? balanceData.toFixed(2) : balanceData; // Handle "N/A"

  //                 // // Center and add Total Excl VAT
  //                 // const pageWidth = doc.internal.pageSize.width;
  //                 // const textWidth = doc.getTextWidth(`Total Excl VAT: ${amountStr}`);
  //                 // const xPosition = (pageWidth - textWidth) / 2;
  //                 // doc.text(`Total Excl VAT: ${amountStr}`, xPosition, finalY + 10);

  //                 // // Center and add Total VAT Amount
  //                 // const textWidthTax = doc.getTextWidth(`Total VAT: ${taxStr}`);
  //                 // const xPositionTax = (pageWidth - textWidthTax) / 2;
  //                 // doc.text(`Total VAT: ${taxStr}`, xPositionTax, finalY + 15);

  //                 // // Center and add Total Incl VAT
  //                 // const textWidthTotal = doc.getTextWidth(`Total Incl VAT: ${totalStr}`);
  //                 // const xPositionTotal = (pageWidth - textWidthTotal) / 2;
  //                 // doc.text(`Total Incl VAT: ${totalStr}`, xPositionTotal, finalY + 20);

  //                 // // Center and add Party Balance
  //                 // const textWidthBalance = doc.getTextWidth(`Party Balance: ${balanceStr}`);
  //                 // const xPositionBalance = (pageWidth - textWidthBalance) / 2;
  //                 // doc.text(`Party Balance: ${balanceStr}`, xPositionBalance, finalY + 25);


  //                 // QR Code
  //                 //doc.addImage(qrCodeDataURL, 'PNG', 85, finalY + 35, 40, 40); // Position QR code

  //                 // Footer
  //                 // doc.text("THANK YOU FOR YOUR CUSTOMERSHIP AND THANKS FOR CHOOSING US", 105, finalY + 90, { align: "center" });
  //                 // doc.text("THIS IS A COMPUTER GENERATED INVOICE SO NO SIGNATURE REQUIRED", 105, finalY + 95, { align: "center" });
  ...
  //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  onClickPrint(selectedSalesId: number): void {
    this.pdfgenerationService.generatePdf(selectedSalesId, this.pageId, 'print', this.isMobile, this.isTab,null);
