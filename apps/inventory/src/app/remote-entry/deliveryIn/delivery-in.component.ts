import { additonalChargesPopup, bankPopup, cardPopup, cashPopup, chequePopup, Currency, Customer, DeliveryLocation, GridSettings, ItemOptions, Items, itemTransaction, PayType, Projects, Purchase, Purchases, Reference, Salesman, SizeMaster, StockItems, Supplier, taxPopup, UnitPricePopup, VehicleNo, VoucherType, Warehouse } from '../model/purchase.interface';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { DeliveryInService } from '../../services/deliveryin.service';
...
import { Account, Branches, Category, CountryOfOrigin, ItemBrand, ItemColor, ItemHistory, ItemUnitDetails, parentItem, Quality, TaxType, UnitData } from '../model/itemmaster.interface';
...
@Component({
  selector: 'dfinance-frontend-delivery-in',
  templateUrl: './delivery-in.component.html',
  styleUrls: ['./delivery-in.component.css'],
})
export class DeliveryInComponent {
  ...
  alldeliveryInList = [] as Array<Purchases>;   //leftside fill
  tempSalesList: any = [];
  selectedDelInId!: number;
  firstSales!: number;
  expiryitem = true;
  //rawmaterials = true;
  //finishedgoods = true;
  currentSalesEstimate = {} as Purchase;
...
/// Data Table variable
  ...
  constructor(
    ...
    private DeliveryInService: DeliveryInService,
  ...
  ngOnInit(): void {
    this.DeliveryInForm = this.formBuilder.group({
    ...
    this.subscription = interval(60000) // 5 minutes in milliseconds
      .subscribe(() => {
        this.fetchAllDeliveryIn(); // Call the function every 5 minutes
      });
    }
    ...
    this.DeliveryInService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.alldeliveryInList = response?.data;
          this.tempSalesList = [...this.alldeliveryInList];
          this.firstSales = this.alldeliveryInList[0].ID;
        },
        error: (error) => {
          // this.isLoading = false;
  ...
  onClickNewDeliveryIn() {
    ...
    this.DeliveryInForm.reset();
    this.additionalDetailsForm.reset();
    this.selectedDelInId = 0;
    this.updatedCustomer = "";
    this.updatedProject = "";
    this.itemDetails = [];
    this.isDelInCancelled = false;
    ...
    if (this.isInputDisabled == true) {
      this.disbaleFormControls();
      this.selectedDelInId = this.firstSales;
      this.fetchDeliveryInById();
    } else {
      this.selectedDelInId = 0;
      this.DeliveryInForm.patchValue({
      ...
      .subscribe({
        next: (response) => {
          ...
          if (Array.isArray(responseData) && responseData.length > 0) {
            ...
            this.onCustomerSelected(responseData[0].accountID);
          }
        },
        error: (error) => {
          console.error('An Error Occured', error);
  ...
  // onChangePayType() {
  //   let paytypeId = this.DeliveryInForm.get('paytype')?.value;
  ...
  saveCurrencyRate(){
    ...
    if(confirm('Are you sure you want to update exchange rate for this currency?')){
      this.DeliveryInService.updateDetails(EndpointConstant.UPDATEEXCHANGERATE+this.selectedCurrencyId+'&exchRate='+this.currentCurrencyRate, {})
        .pipe(takeUntil(this.destroySubscription))
        .subscribe({
          next: (response) => {
  ...
  fetchItemFillData() {
    this.partyId = this.selectedPartyId;
    this.locId = this.DeliveryInForm.get('warehouse')?.value;
    this.DeliveryInService
...
setItemDetailsFromImportReference(){
  ...
  if(this.importedReferenceList.length > 0 && !this.isReferenceImported){
    this.importedReferenceList.forEach((element:any) => {
      ...
      if (!itemExists) {
        ...
        insertItem.taxPerc = element.TaxPerc;
        insertItem.taxValue = this.baseService.formatInput(Number(element.TaxValue));
        insertItem.total = 0.0000;
        insertItem.expiryDate = element.ExpiryDate ? element.ExpiryDate : null;
  ...
  enableFormControls() {
    this.DeliveryInForm.get('purchasedate')?.enable();
    this.DeliveryInForm.get('reference')?.enable();
    this.DeliveryInForm.get('customer')?.enable();
    this.DeliveryInForm.get('currency')?.enable();
    this.DeliveryInForm.get('exchangerate')?.enable();
    this.DeliveryInForm.get('warehouse')?.enable();
    this.DeliveryInForm.get('project')?.enable();
    this.DeliveryInForm.get('description')?.enable();
    this.DeliveryInForm.get('approve')?.enable();
    this.DeliveryInForm.get('terms')?.enable();
    this.DeliveryInForm.get('totaldiscpercent')?.enable();
    this.DeliveryInForm.get('discountamount')?.enable();
    this.DeliveryInForm.get('roundoff')?.enable();
    this.DeliveryInForm.get('netamount')?.enable();
    this.DeliveryInForm.get('grandtotal')?.enable();
    this.DeliveryInForm.get('paytype')?.enable();
    this.DeliveryInForm.get('totalpaid')?.enable();
    this.DeliveryInForm.get('balance')?.enable();
    this.DeliveryInForm.get('duedate')?.enable();
    this.DeliveryInForm.get('additemcharges')?.enable();
    this.DeliveryInForm.get('salesman')?.enable();
    this.DeliveryInForm.get('vatno')?.enable();
    this.DeliveryInForm.get('partyinvoiceno')?.enable();
    this.DeliveryInForm.get('partyinvoicedate')?.enable();
  ...
  disbaleFormControls() {
    this.DeliveryInForm.get('purchasedate')?.disable();
    this.DeliveryInForm.get('reference')?.disable();
    this.DeliveryInForm.get('customer')?.disable();
    this.DeliveryInForm.get('currency')?.disable();
    this.DeliveryInForm.get('exchangerate')?.disable();
    this.DeliveryInForm.get('warehouse')?.disable();
    this.DeliveryInForm.get('project')?.disable();
    this.DeliveryInForm.get('description')?.disable();
    this.DeliveryInForm.get('approve')?.disable();
    this.DeliveryInForm.get('terms')?.disable();
    this.DeliveryInForm.get('totaldiscpercent')?.disable();
    this.DeliveryInForm.get('discountamount')?.disable();
    this.DeliveryInForm.get('roundoff')?.disable();
    this.DeliveryInForm.get('netamount')?.disable();
    this.DeliveryInForm.get('grandtotal')?.disable();
    this.DeliveryInForm.get('paytype')?.disable();
    this.DeliveryInForm.get('totalpaid')?.disable();
    this.DeliveryInForm.get('balance')?.disable();
    this.DeliveryInForm.get('duedate')?.disable();
    this.DeliveryInForm.get('additemcharges')?.disable();
    this.DeliveryInForm.get('salesman')?.disable();
    this.DeliveryInForm.get('vatno')?.disable();
    this.DeliveryInForm.get('partyinvoiceno')?.disable();
    this.DeliveryInForm.get('partyinvoicedate')?.disable();
  ...
  onClickSaveDeliveryIn() {
    if(this.DeliveryInForm.get('customer')?.value == null){
    ...
    const date=this.DeliveryInForm.get('purchasedate')?.value
    if(date === null){
      alert("Enter date and proceed")
    }
  
    //fetch project data...
     if (this.DeliveryInForm.value.project) {
      this.projectData.forEach((element:any)=>{
        if(element.projectname == this.DeliveryInForm.value.project){
    ...
    //set warehouse data..
    if(this.DeliveryInForm.get('warehouse')?.value){
      this.warehouseData.forEach((element:any)=>{
        if(element.id == this.DeliveryInForm.get('warehouse')?.value){
    ...
    //set salesman data...
    if(this.DeliveryInForm.get('salesman')?.value){
      this.salesmanData.forEach((element:any)=>{
        if(element.name == this.DeliveryInForm.get('salesman')?.value){
    ...
    const payload = {
      "id": this.selectedDelInId ? this.selectedDelInId : 0,
      "voucherNo": this.formVoucherNo,
      "date": this.DeliveryInForm.value.purchasedate,
      "reference": this.DeliveryInForm.value.reference,
      ...
      "description":  this.DeliveryInForm.value.description,
      "grossAmountEdit": this.isgrossAmountEditable,
      "fiTransactionAdditional": {
        "transactionId": 0,
        "terms": this.DeliveryInForm.value.terms,
        ...
        "payType": {},
        "approve": this.DeliveryInForm.value.approve,
        "days": 0,
        "closeVoucher": true,
        "code": "string",
      ...
      "transactionEntries": {
       "totalDisc": this.discountTotal,
        ...
        "netAmount": this.DeliveryInForm.value.netamount,
        "grandTotal": this.grandTotal,
        "payType": {},
        "dueDate": this.DeliveryInForm.value.duedate,
        "totalPaid": this.DeliveryInForm.value.totalpaid,
        "balance": this.DeliveryInForm.value.balance,
        "advance": [],
        "cash":[],
        "card": [],
        "cheque": [],
    ...
    console.log("items="+JSON.stringify(this.itemDetails, null, 2));
    if (this.isUpdate) {
  ...
  updateCallback(payload: any) {
    ...
    this.DeliveryInService.updateDetails(EndpointConstant.UPDATEDELIVERYIN+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  createCallback(payload: any) {
    //console.log("Payload=", JSON.stringify(payload, null, 2));
    this.DeliveryInService.saveDetails(EndpointConstant.SAVEDELIVERYIN+this.pageId+'&voucherId='+this.voucherNo, payload)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.isLoading = false;
  ...
  onClickDeleteDeliveryIn(event:Event) {
  ...
  confirmDelete(){
    this.DeliveryInService.deleteDetails(EndpointConstant.DELETEPURCHASE + this.selectedDelInId+'&pageId='+this.pageId)
        .pipe(takeUntil(this.destroySubscription))
        .subscribe({
          next: (response) => {
  ...
  onClickCancelDeliveryIn(event:Event){
    event.preventDefault();
    if(!this.isCancel){
      alert('Permission Denied!');
      return false;
  ...
  confirmCancel(){
    this.DeliveryInService.updateDetails(EndpointConstant.CANCELPURCHASE + this.selectedDelInId+'&pageId='+this.pageId+'&reason='+this.cancelReason,{})
        .pipe(takeUntil(this.destroySubscription))
        .subscribe({
          next: (response) => {
            const status = response.httpCode as keyof typeof STATUS_MESSAGES;
  ...
  onClickDeliverIn(event: any): void {
    if (event.type === 'click') {
      ...
      this.selectedDelInId = event.row.ID;
      console.log(this.selectedDelInId)
      this.emptyAllSummaryTotalsAndObjects();
      this.fetchDeliveryInById();
    }
  }
  ...
  fetchDeliveryInById(): void {
    //this.isLoading = true;
    this.DeliveryInService
      .getDetails(EndpointConstant.FILLPURCHASEBYID + this.selectedDelInId + '&pageId=' + this.pageId)
      .pipe(takeUntil(this.destroySubscription))
      .subscribe({
        next: (response) => {
          this.currentSalesEstimate = response?.data;
  ...
  FillPurchaseDetails(){
    let transactionDetails = this.currentSalesEstimate?.transaction.fillTransactions;
    let transactionAddditional = this.currentSalesEstimate?.transaction.fillAdditionals;
    this.invTransactions = this.currentSalesEstimate?.transaction.fillInvTransItems;
    let transactionEntries:any = this.currentSalesEstimate?.transaction.fillTransactionEntries;

    //fill data from transexpanse 
    let transexpenses:any=this.currentSalesEstimate?.transaction.fillTransactionExpenses;

    
    this.referenceData = this.currentSalesEstimate?.transaction.fillVoucherAllocationUsingRef;
    this.DeliveryInForm.patchValue({
            vouchername:this.vocherName,
    ...
    this.isDelInCancelled = transactionDetails.cancelled;
    //set customer data...
  ...
  //     });
  //     this.DeliveryInForm.patchValue({
  //       "card":this.baseService.formatInput(this.cardAmount),
  //       "cash":this.baseService.formatInput(this.cashAmount),
  //       "cheque":this.baseService.formatInput(this.chequeAmount)
  //     });
  //   }
  // }
  ...
  //     this.DeliveryInForm.patchValue({
  //       "advance":this.referenceData.amount
  //     })
  ...
  checkItemCodeCanAdd() {
    const warehouseId = this.DeliveryInForm.get('warehouse')?.value;
    const customerId = this.DeliveryInForm.get('customer')?.value;
  ...
  // fetchBatchNoPopup(itemId:any){
  //   let warehouseId = this.salesEstimateForm.get('warehouse')?.value;
  ...
  //       let batchNoOptions = response.data;
  //     },
  //     error: (error) => {
  //       console.error('An Error Occured', error);
  //     },
  ...
  fetchBatchNoPopup(itemId: any): Observable<any> {
    let warehouseId = this.DeliveryInForm.get('warehouse')?.value;
    return this.DeliveryInService
      .getDetails(EndpointConstant.FETCHBATCHNOFORSALES + warehouseId + '&ItemID=' + itemId)
      .pipe(
  ...
  calculateItemAmount(rowIndex: number) {
    ...
    if (taxValue && taxValue != oldTaxValue && 942) {
  ...
  onChangeTaxType(): void {
    const selectedTaxTypeId = this.DeliveryInForm.get('taxtype')?.value;
  ...
  onChangeQuality(): void {
    const selectedQualityId = this.DeliveryInForm.get('quality')?.value;
  ...
  onStockItemChange(event: Event) {
    ...
    if (this.isStockItem == false) {
      this.DeliveryInForm.get('invaccount')?.enable();
      this.DeliveryInForm.get('salesaccount')?.enable();
      this.DeliveryInForm.get('costaccount')?.enable();
      this.DeliveryInForm.get('purchaseaccount')?.enable();
    } else {
      this.DeliveryInForm.get('invaccount')?.disable();
      this.DeliveryInForm.get('salesaccount')?.disable();
      this.DeliveryInForm.get('costaccount')?.disable();
      this.DeliveryInForm.get('purchaseaccount')?.disable();
    }
  }
  ...
  translateItemName() {
    let itemName = this.DeliveryInForm.get('itemname')?.value;
    ...
    this.DeliveryInService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.onCustomerSelected(Number(transactionDetails.AccountID));
  ...
  checkAdvanceAmount() {
    let voucherDate = this.DeliveryInForm.get('purchasedate')?.value;
    if (voucherDate == null) {
      alert('Invalid Date');
      return false;
    }
  ...
  onClickRoundOff(){
    this.roundValue = Number(this.DeliveryInForm.get('roundoff')?.value);
 ...
 @HostListener('window:keydown', ['$event'])
  onBodyKeydown(event: KeyboardEvent): void {
    if (event.ctrlKey && (event.key === 'p' || event.key === 'P')) {
      ...
      if(this.selectedDelInId == 0 && !this.isInputDisabled){
        if(this.balanceAmount > 0){
          this.openDialog();
        } else{
          alert('Amount must be greater than zero');
        }
      } else if(this.selectedDelInId > 0 && !this.isInputDisabled){
        this.activePrintOption = true;
        this.onClickSaveDeliveryIn();
      }     
      ...
      if(!this.isDeleteBtnDisabled){
        this.onClickDeleteDeliveryIn(event);
      }      
